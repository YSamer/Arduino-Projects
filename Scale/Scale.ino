...
void loop() {
  ...
  if(Screen==0){  //Show Scale Screen
    delay(500);
    if(checkKey()==0){zeros();}
  
    if(checkKey()==12){  //Config. Key
      if(millis()-keyTime>=3000){   //Config. Key pressed Time
         Screen = 1;
      }
    }else{
      keyTime = millis();
  ...
  if(Screen==1){//Menue Screen
    if(S1==0){
      shift(0x0b, 0x06); shift(6, 14); shift(5, 0); shift(4, 15); shift(3, 15); shift(2, 15); shift(1, 15); //P0
    }
    if(S1==1){
      shift(0x0b, 0x06); shift(6, 14); shift(5, 1); shift(4, 15); shift(3, 15); shift(2, 15); shift(1, 15); //P1
    }
    if(S1==2){
      shift(0x0b, 0x06); shift(6, 14); shift(5, 2); shift(4, 15); shift(3, 15); shift(2, 15); shift(1, 15); //P2
    }
    if(S1==3){
      shift(0x0b, 0x06); shift(6, 14); shift(5, 3); shift(4, 15); shift(3, 15); shift(2, 15); shift(1, 15); //P3
    }
    if(S1==4){
      shift(0x0b, 0x06); shift(6, 14); shift(5, 4); shift(4, 15); shift(3, 15); shift(2, 15); shift(1, 15); //P4
    }
    if(S1==5){
      shift(0x0b, 0x06); shift(6, 14); shift(5, 5); shift(4, 15); shift(3, 15); shift(2, 15); shift(1, 15); //P5
    }
