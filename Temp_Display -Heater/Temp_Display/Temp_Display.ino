...
if(layer == 0){
 {
  ...
  if(getTemp()<=(getOTemp()-difDT)&&getTemp()<=(getOTemp())&&Try<=3){
    ...
    while(millis()-lHTime<=1000){
      digitalWrite(R1,LOW);
      digitalWrite(R2,LOW);
      digitalWrite(R3,HIGH);
      Display();
    }while((millis()-lHTime>=1000)&&(millis()-lHTime<=4000)){
      digitalWrite(R1,LOW);
      digitalWrite(R2,HIGH);
      digitalWrite(R3,HIGH);
      Display();
    }while((millis()-lHTime>=4000)&&(millis()-lHTime<=6000)){
      ...
      if((millis()-lHTime>=9000)&&analogRead(cell)>940){
        if(millis()-noFireT>=2000){
          digitalWrite(R1,LOW);
          digitalWrite(R2,LOW);
          digitalWrite(R3,LOW);
          noFireState = 1;
      ...
      Display();
      if((millis()-lHTime>=(9000+(5000*Try))) && analogRead(cell)>940 && noFireState==1){
        lHTState=0;
        Try++;
      }if(analogRead(cell)<940){
        Try=1;
        noFireState = 0;
      }
    }
